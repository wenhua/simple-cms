(function() {
  ...
  write = function(input_path, name, contents, output_directory, ext) {
    ...
    return path.exists(dir, function(exists) {
      ...
      return fs.writeFile(output_path, contents, function(err) {
        handle_error(err);
        if (options.print) puts(contents);
        if (options.watch) return puts("Compiled " + input_path);
      });
    });
  };
  ...
  this.run = function() {
    ...
    if (options.help || argv.length === 0) puts(parser.help());
    if (options.version) puts(coffeekup.version);
    if (options.utils) {
